# Backrooms Redesign Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Rewrite Path 8 (The Backrooms) from 15 rooms/3 stages to 100 rooms/4 stages with digital infrastructure horror theme, real browser data, decayed portal elements, and cosmetic URL bar manipulation.

**Architecture:** All changes are in `html/index.html`. `scene_backrooms()` is rewritten to dispatch to four stage functions (`brStage1`–`brStage4`). Helper functions handle browser data, URL path building, and formatting. Templates within each stage are selected by `slot % 5`. Landmark rooms are checked first with explicit slot comparisons and short-circuit before template selection.

**Tech Stack:** Vanilla JS, plain CSS, no build system. Open `html/index.html` directly in a browser to test. No npm, no server required.

---

### Task 1: CSS and State Setup

**Files:**
- Modify: `html/index.html` — CSS `<style>` block around line 499 (the `/* ── backrooms ── */` section)
- Modify: `html/index.html` — `S` object around line 509 (after `brVisited: false,`)

- [ ] **Step 1: Replace the backrooms CSS block**

Find this exact text near line 499:
```css
/* ── backrooms ── */
body.br-mode{background:#e8d5a3}
@keyframes br-flicker{0%,94%,96%,100%{filter:brightness(1)}95%{filter:brightness(.85)}}
```
...
/* ── backrooms ── */
body.br-mode{background:#e8d5a3}
body.br-deep{background:#0d0a06}
@keyframes br-flicker{0%,94%,96%,100%{filter:brightness(1)}95%{filter:brightness(.85)}}
@keyframes br-deep-flicker{0%,88%,91%,95%,100%{filter:brightness(1)}89%{filter:brightness(.6)}94%{filter:brightness(.85)}}
```

- [ ] **Step 2: Add new state fields to S**

Find `brVisited: false,` in the `S` object. Add two fields immediately after it:
```js
brVisited: false,
brBattery: null,
brUrlPath: '',
...
Open `html/index.html`. Open DevTools console. Run:
```js
document.body.classList.add('br-deep');
```
Page background should go near-black (`#0d0a06`). Run:
```js
document.body.classList.remove('br-deep');
```
Background returns to normal. Also confirm:
```js
S.brBattery  // null
S.brUrlPath  // ""
```
...
git commit -m "backrooms: add br-deep CSS and state fields"
```

---

### Task 2: Browser Data Helper Functions

**Files:**
- Modify: `html/index.html` — add 5 helper functions just before `scene_backrooms_enter()` (~line 6060)

- [ ] **Step 1: Add helper functions**

Insert this block immediately before `function scene_backrooms_enter()`:
...
```js
brBrowserName()          // "Chrome", "Firefox", "Safari", or "Edge"
brTimezoneRegion()       // e.g. "america", "europe", "asia"
brFormatDuration(75000)  // "1m 15s"
brFormatDuration(45000)  // "45s"
brHex(255, 6)            // "0000FF"
brFaultCode(42)          // "0x..." something reasonable
```

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: add browser data helper functions"
```

---

### Task 3: URL Path Builder

**Files:**
- Modify: `html/index.html` — add `brBuildUrl()` and its segment pool constants immediately after the helpers from Task 2

- [ ] **Step 1: Add URL builder**

Insert this block immediately after the helpers from Task 2 (still before `scene_backrooms_enter()`):
...
S.brUrlPath = '';
brBuildUrl(1, 1, 1);
location.pathname  // "/dashboard/export/diagnostic/session/repair"

brBuildUrl(1, 2, 2);
location.pathname  // should have appended "/manifest" (BR_URL_S1_SEGS[2 % 8])

S.brUrlPath = '';
brBuildUrl(2, 26, 26);
location.pathname  // "/cdn-edge/[your-region]/cache/layer..."
```

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: add URL path builder"
```

---

### Task 4: Update scene_backrooms_enter()

**Files:**
- Modify: `html/index.html` — `scene_backrooms_enter()` function (~line 6060)

- [ ] **Step 1: Replace scene_backrooms_enter() entirely**

Find the entire `function scene_backrooms_enter() { ... }` body and replace it with:
...
Navigate: Dashboard → sidebar "Export" → wait 45 seconds for the bar to overflow to 107% → click "Open diagnostic console →".

The boot log should display your actual browser name (e.g. "Chrome"), your OS platform, and your actual viewport dimensions (e.g. "1440×900") in lines 8 and 9. After the final line "descending…", Room 1 should load after ~700ms.

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: update enter sequence with real browser data and battery fetch"
```

---

### Task 5: brStage1() — The Diagnostic Surface

**Files:**
- Modify: `html/index.html` — add `brStage1()` immediately before `scene_backrooms_enter()`

This function handles slots 1–25. Landmark slots (1, 3, 7, 12, 15, 18, 22, 25) are checked first; all other slots fall through to templates selected by `slot % 5`.
...
```js
root.innerHTML = brStage1(1, '00001', 1);
// Should show: "Diagnostic Console / No errors found. / (you have been here before.)"

root.innerHTML = brStage1(3, '00003', 3);
// Should show: cookie banner with session_ghost, render_id_orphan, consent_cache_stale

root.innerHTML = brStage1(7, '00007', 7);
// Should show: pricing table with $NaN/mo, $undefined, Contact [object Object]

root.innerHTML = brStage1(12, '00012', 12);
// Should show: "Step 8 of 3"

root.innerHTML = brStage1(15, '00015', 15);
// Should show: black box with 95% loading bar

root.innerHTML = brStage1(22, '00022', 22);
// Should show: your actual browser name and viewport

root.innerHTML = brStage1(25, '00025', 25);
// Should show: "Fault detected in layer below" with monospace button
```

Also spot-check 2 template slots (any non-landmark slots like 2, 4, 5, 6).

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: Stage 1 rooms (diagnostic surface, 25 rooms)"
```

---

### Task 6: brStage2() — The Network Layer

**Files:**
- Modify: `html/index.html` — add `brStage2()` immediately after `brStage1()`
...
root.innerHTML = brStage2(26, '00026', 26);
// Warm sepia background; "You have passed the application layer."

root.innerHTML = brStage2(30, '00030', 30);
// Long changelog, all entries "User navigated deeper." with timestamps

root.innerHTML = brStage2(50, '00050', 50);
// Full-width access log; last two lines in different colors

root.innerHTML = brStage2(46, '00046', 46);
// Should show your real connection type, language, timezone
```

Check that letter-spacing visibly widens between slot 26 and slot 49.

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: Stage 2 rooms (network layer, 25 rooms)"
```

---

### Task 7: brStage3() — Server / Data Horror

**Files:**
- Modify: `html/index.html` — add `brStage3()` immediately after `brStage2()`
...
```js
document.body.classList.add('br-mode');
root.innerHTML = brStage3(51, '00051', 51);
// Dark (#1a1410) background; "What follows is not a product."

root.innerHTML = brStage3(55, '00055', 55);
// Shows real elapsed time since page load as "Xm Ys"

root.innerHTML = brStage3(58, '00058', 58);
// Shows your real timezone region in social proof

root.innerHTML = brStage3(75, '00075', 75);
// Checklist with real browser/viewport/history data
```

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: Stage 3 rooms (server/data horror, 25 rooms)"
```

---

### Task 8: brStage4() — The Void

**Files:**
- Modify: `html/index.html` — add `brStage4()` immediately after `brStage3()`
...
```js
document.body.classList.add('br-mode','br-deep');
root.innerHTML = brStage4(76, '00076', 76);
// Near-black bg; shows real elapsed duration; exit input visible

root.innerHTML = brStage4(88, '00088', 88);
// "the code is" then invisible text. Select with mouse — should select "LEVEL0"

root.innerHTML = brStage4(96, '00096', 96);
// Text barely visible; "(you never left.)"

// T4-A template — check any slot like 77 (77 % 5 = 2) or 78 (78 % 5 = 3):
root.innerHTML = brStage4(77, '00077', 77);
root.innerHTML = brStage4(80, '00080', 80);
// Careers confirmation fragment
```

Verify exit input: type `LEVEL0` → nothing happens yet (scene_backrooms_exit isn't updated). Type anything else → input border briefly turns red.

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: Stage 4 rooms (the void, 25 rooms)"
```

---

### Task 9: Rewrite scene_backrooms() Dispatch

**Files:**
- Modify: `html/index.html` — `scene_backrooms()` function (~line 6100)

- [ ] **Step 1: Replace scene_backrooms() entirely**

Find the full existing `function scene_backrooms(roomArg) { ... }` body. Replace it with:
...
- [ ] **Step 2: Full path walkthrough in browser**

Navigate: Dashboard → sidebar Export → wait 45s → click "Open diagnostic console →".

Walk through these rooms and verify:

| Room | Expected |
|------|----------|
| 1 | "No errors found. / (you have been here before.)" White SaaS UI |
| 3 | Cookie banner: session_ghost, render_id_orphan, consent_cache_stale |
| 7 | Pricing: $NaN/mo, $undefined, Contact [object Object] |
| 12 | "Step 8 of 3" |
| 15 | Black box, 95% stuck, "before you arrived" |
| 25 | "Fault detected in layer below" — monospace button appears |
| 26 | Sepia bg starts; "You have passed the application layer" |
| 30 | Changelog: all entries "User navigated deeper" |
| 50 | Full log room; "you are reading this log / already written" |
| 51 | Dark bg (#1a1410); "What follows is not a product" |
| 55 | Real elapsed time shown |
| 75 | Onboarding checklist with real data, all red |
| 76 | Near-black; exit input appears |
| 88 | "the code is [invisible LEVEL0]" |
| 100 | "end of indexed range. continuing." |
| 101 | Loops to slot 1 content; display number ~950101 |

Also verify URL bar updates with each room — should grow longer and show stage-appropriate paths.

- [ ] **Step 3: Commit**
```bash
git add html/index.html
git commit -m "backrooms: rewrite scene_backrooms() dispatch for 100-room cycle"
```

---

### Task 10: Exit and Survey Cross-Path

**Files:**
- Modify: `html/index.html` — `scene_backrooms_exit()` (~line 6260), survey Q19 line (~line 4784)

- [ ] **Step 1: Update scene_backrooms_exit()**

Find the existing `function scene_backrooms_exit()` and replace it with:
...
- [ ] **Step 2: Update survey Q19 cross-path**

Find this line (~line 4784):
```js
const opts = (S.surveyStep === 18 && S.brVisited) ? [...q.opts, 'Yes. Something is in the walls. The code is LEVEL0.'] : q.opts;
```
...
1. Enter backrooms (Dashboard → Export → 45s → enter).
2. Navigate to Stage 4 (room 76+). Type `LEVEL0` in exit input → click submit.
3. Confirm: toast "Export complete", redirect to dashboard, URL bar shows `/`, `br-deep` class removed from `document.body`.

- [ ] **Step 4: Verify survey cross-path (before room 76)**

1. Enter backrooms but stop at room ~30 (still Stage 2).
2. Navigate to Footer → Survey → answer through to Q19 (question 19, the "being observed" question — `S.surveyStep === 18`).
3. Verify the extra option reads `"Yes. Something is in the walls. The code is LEVEL0."`

- [ ] **Step 5: Verify survey cross-path (after room 76)**

1. Enter backrooms, navigate past room 76.
2. Navigate to Footer → Survey → Q19.
3. Verify the extra option now reads `"Yes. The code is LEVEL0. You already know this."`

- [ ] **Step 6: Commit**
```bash
git add html/index.html
git commit -m "backrooms: exit URL reset, br-deep cleanup, enhanced survey Q19 cross-path"
```

---

### Task 11: Remove Old Code and Final Verification

**Files:**
- Modify: `html/index.html` — confirm no old 15-room code remains

- [ ] **Step 1: Search for old 15-room cycle formula**

In the file, search for the string `% 15`. The old formula was `((roomArg - 1) % 15) + 1`. This should not appear anywhere in `scene_backrooms()`. If it does, delete the stale function body.

```bash
grep -n "% 15" html/index.html
```

Expected output: no results (or results only in unrelated code). If found in the backrooms section, delete those lines.

- [ ] **Step 2: Confirm router entries are intact**

Search for the `SCENES` object (~line 6270). Verify these entries exist and are correct:
```js
backrooms_next: scene_backrooms_next,
backrooms_exit: scene_backrooms_exit,
```

`scene_backrooms_next()` should still be the one-liner:
```js
...
Open `html/index.html` in browser. Complete this checklist:

- [ ] Enter the backrooms via: Dashboard → sidebar Export → wait 45s → "Open diagnostic console →"
- [ ] Boot log shows real browser name, platform, viewport
- [ ] Navigate through rooms 1→3→7→12→15→18→22→25 — each landmark renders correctly
- [ ] Room 26: sepia background begins, "You have passed the application layer"
- [ ] Room 30: changelog with identical timestamped entries
- [ ] Room 35: roadmap with "In Transit / Routing / Lost in Queue"
- [ ] Room 38: auto-ticket opened and closed with display room number
- [ ] Room 42: stuttering terms text, [Recede] and [Continue] both advance
- [ ] Room 46: your real connection type, language, timezone displayed
- [ ] Room 50: log room, last two lines in different colors
- [ ] Room 51: dark background (#1a1410)
- [ ] Room 55: real elapsed session time
- [ ] Room 58: your real timezone region in social proof
- [ ] Room 62: confirmation prompt with real room number
- [ ] Room 66: survey Q19 floating with radio options
- [ ] Room 70: Jordan/Brad transcript fragments
- [ ] Room 75: onboarding checklist with real data, all red
- [ ] Room 76: near-black, exit input visible
- [ ] Room 80: "[undefined]" careers confirmation
- [ ] Room 84: just the faded wordmark
- [ ] Room 88: "the code is" — select the invisible text, confirm it says `LEVEL0`
- [ ] Room 92: "you have been here before / you will be here again"
- [ ] Room 96: mirror of room 1, "(you never left.)"
- [ ] Room 100: "end of indexed range. continuing."
- [ ] Room 101: display number shows ~950101, content is room 1 (diagnostic console) again
- [ ] URL bar: grows longer with each room, never shortens, shows stage-appropriate path format
- [ ] Exit `LEVEL0`: returns to dashboard, URL resets to `/`, background returns to normal
- [ ] Survey Q19 (after entering backrooms, before room 77): hidden option shows "Something is in the walls. The code is LEVEL0."
- [ ] Survey Q19 (after room 77): hidden option shows "You already know this."

- [ ] **Step 4: Commit**
```bash
git add html/index.html
git commit -m "backrooms: final cleanup and verification"
```
